...
import models_mae, models_milan
...
def get_args_parser():
    ...
    parser.add_argument('--change_decoder', action='store_true', default=True) # change the decoder model
...
def main(args):
    ...
    dataset_train = ImageFolderInstance(os.path.join(args.data_path, 'train'), transform=transform_train)
    print(dataset_train)
    ...
    # define the model
    if args.use_ablation_model is False:
        model = models_mae.__dict__[args.model](norm_pix_loss=args.norm_pix_loss, use_clip=args.use_clip) # attn_mask is used by default
        if args.change_decoder:
            print('Change to MILAN model')
            model = models_milan.__dict__[args.model](norm_pix_loss=args.norm_pix_loss, use_clip=args.use_clip, attn_mask=args.attn_mask)
    else:
        print('Use ablation model')
        model = models_mae_ablation.__dict__[args.model](norm_pix_loss=args.norm_pix_loss, use_clip=args.use_clip, attn_mask=args.attn_mask)
    ...
    if args.use_clip:
        if args.use_clip_slip is False and args.use_clip_dino is False:
            ...
            def process_state_dict(state_dict):
                ...
                return state_dict
            clip_model = build_slip_model(process_state_dict(torch.load('./slip_vit_base_16.pth.tar', map_location='cpu')['state_dict']), 'base')
